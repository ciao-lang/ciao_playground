...
embed editable and runnable code examples} in manuals, tutorials,
slides, exercises, etc., and in general any kind of document.

These documents can be developed with many tools, such as Google Docs,
Jupyter notebooks, Word, Powerpoint, LaTeX, Pages, Keynote, Org mode,
web site generators, etc., etc.

The examples are stored in the documents themselves and do not need to
be uploaded to (or edited in) any server.

In addition, the LPdoc documenter includes specific commands for
embedding code @em{editable} and @em{runnable} in place or in a Ciao
Playground.
...
@item Paste or upload the example program into the playground editor.

@item Click the @key{Share!} button. This will copy into the clipboard
a link to an instance of the playground with the program included.

@item Go to the document with the exampe and insert the link in the
document as a URL.
...
@section{Creating Documents with Editable and Runnable Examples using LPdoc}
