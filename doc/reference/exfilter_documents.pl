...
:- doc(title, "Creating Interactive Verification Tutorials").

:- doc(module,"
@cindex{verification tutorial}
@cindex{interactive}

This part describes advanced uses of LPdoc and the Ciao Playground to
create and maintain interactive tutorials for analysis and
verification using CiaoPP.

These documents require the features provided in the @apl{exfilter}
bundle, which extends LPdoc and the playground with functionality to
extract, filter, and compare the output of program analyses, both
statically and dynamically.

@section{Non-interactive examples (static content)}

Messages and output generated during @apl{Ciao} compilation or
@apl{CiaoPP} analysis, for a particular code snippet, can be filtered
and included in manuals with the following LPdoc command:

 @tt{@@}@em{exfilter}@tt{@{}@var{Code}@tt{@}@{}@var{Action}, @var{FlagsValues}, @var{Filters}@tt{@}}.

In order to enable this functionality it is necessary to include this
line in the LPdoc document configuration files (e.g.,
@tt{SETTINGS.pl}):
```
load_doc_module := exfilter(exfilter_lpdoc).
```

This process happens in two phases: first the document is processed to
identify the different (compilation, analysis, verification, etc.)
tasks, then the @apl{ciao-exfilter} command needs to be executed to
process the tasks and filter the results necessary for the
manual. Typically the results are statically committed in the
respository, which enables easy detection of regressions.
...
where the @tt{pred} assertion indicates how @pred{app/3} should be
called in this program. We would like to display the part of the
analysis output that shows if the assertion has been checked.  This
fragment can be generated by including in the source file the
following code:
...
where @var{FlagName} is a valid @apl{CiaoPP} flag name.  This list is
optional and the flags not included in this list will be assumed to
take their default value.  Check
@href{/ciao/build/doc/ciaopp.html/}{CiaoPP Reference Manual} for the
...
@section{Interactive examples (dynamic content)}
...
When users submit their answers, the playground will return
appropriate feedback showing the error messages related to the
singleton variables.  We have defined lpdoc-style commands which mark
different parts of the program.  The playground identifies the
different parts and classifies them. For example, the playground will
recognize a solution, a set of filters that we want to apply, and the
part of the interactive code that will be shown in the documentation.
In this case, the solution will be shown only when the @key{Show
solution} button is clicked. The code part is always visible. Hence,
the previous exercise can be generated by including in the source file
the following code:
...
For the inclusion of exercises in the documentation three distinct
modes of exercises have been developed:
@begin{itemize}
@item @tt{solution=equal} : Prints out the clauses from the user's
  answer that do not match with the file that contains the
  solution. It takes care of trivial differences such as different
  variable names and different formatting of the code in the files.
@item @tt{solution=errors} : Another task is to find bugs in a program
  and fix them. Users submit their solutions and the filter checks
  them. If there are none then the program has been corrected
  successfully.  Additionally, a message filter has been created to
  extract a message or messages by a certain term.
@item @tt{solution=verify_assert} : Analysis information allows us to
  conclude that the program is incorrect or incomplete, i.e., that the
  program does not satisfy the requirements. The idea is that given
  the solution of the user, the filter checks if the user's program
  verifies the assertions (specifications).
@end{itemize}
...
%! \\end{opts}  
``` 
This block runs analysis on the program with a determinism domain and
displays the @tt{true} assertions including @em{comp} properties.
").